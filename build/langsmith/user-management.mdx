...
<Note>
Spaces in the **organization role name** portion of a group name are optional and may be omitted. This is useful for identity providers that disallow spaces in group names. For example, `OrganizationAdmins` and `OrganizationUser` are accepted equivalently to `Organization Admins` and `Organization User`. This flexibility applies **only** to the organization role name token — spaces in workspace names and workspace role names are treated as literal characters and are not equivalent to their space-omitted counterparts.
</Note>
