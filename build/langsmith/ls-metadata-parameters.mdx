...
The filter checks for the **presence of the key**, not truthiness. `{LS_MESSAGE_VIEW_EXCLUDE: False}` still excludes the run. Omit the key entirely to include the run. Child runs that execute inside a `@traceable` (Python) or `traceable` (JS) parent inherit the exclusion through the shared tracing context.
