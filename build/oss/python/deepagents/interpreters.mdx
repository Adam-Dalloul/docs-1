...
When an agent has [subagents](/oss/python/deepagents/subagents) and interpreter middleware, the interpreter exposes a built-in `task()` global that dispatches subagents from code. The agent writes a loop that fans the work out, and all of it runs, so orchestration is deterministic rather than the model choosing whether to spawn one subagent at a time.
